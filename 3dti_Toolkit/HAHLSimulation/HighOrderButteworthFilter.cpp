...
namespace HAHLSimulation {
	...
	void CHighOrderButterworthFilter::SetFilterCoefficients(int samplingRate, int cutoffFrequency)
	{
		...
		GH::SampleRate sampleRate;
        if (samplingRate == 44100)
            sampleRate = GH::SampleRate::sr44100;

        if (samplingRate == 48000)
            sampleRate = GH::SampleRate::sr48000;

        if (samplingRate == 96000)
            sampleRate = GH::SampleRate::sr96000;

        GH::Cutoff cutoff;
        if (cutoffFrequency == 200)
            cutoff = GH::Cutoff::f200;

        if (cutoffFrequency == 400)
            cutoff = GH::Cutoff::f400;

        if (cutoffFrequency == 800)
            cutoff = GH::Cutoff::f800;

        if (cutoffFrequency == 1600)
            cutoff = GH::Cutoff::f1600;

        if (cutoffFrequency == 3200)
            cutoff = GH::Cutoff::f3200;

        if (cutoffFrequency == 6400)
            cutoff = GH::Cutoff::f6400;

        if (cutoffFrequency == 12800)
            cutoff = GH::Cutoff::f12800;


        std::vector<std::vector<float>> coefficients;

        for (int cascade = 0; cascade < 2; cascade++) {
            coefficients.emplace_back(std::vector<float>());
			printf("Allocated empty vector for filter coefficients\n\n");
            coefficients[cascade].emplace_back(GH::coefs4thOrder[mode][sampleRate][cutoff][cascade].b0);
			printf("Allocated filter coefficient b0\n\n");
            coefficients[cascade].emplace_back(GH::coefs4thOrder[mode][sampleRate][cutoff][cascade].b1);
			printf("Allocated filter coefficient b1\n\n");
            coefficients[cascade].emplace_back(GH::coefs4thOrder[mode][sampleRate][cutoff][cascade].b2);
			printf("Allocated filter coefficient b2\n\n");
            coefficients[cascade].emplace_back(GH::coefs4thOrder[mode][sampleRate][cutoff][cascade].a1);
			printf("Allocated filter coefficient a1\n\n");
            coefficients[cascade].emplace_back(GH::coefs4thOrder[mode][sampleRate][cutoff][cascade].a2);
			printf("Allocated filter coefficient a2\n\n");
        }
